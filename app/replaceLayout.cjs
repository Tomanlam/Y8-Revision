...
const dbPath = 'src/components/views/DashboardView.tsx';
let dbData = fs.readFileSync(dbPath, 'utf8');
dbData = dbData.replace(/\{unitProgress\.attemptedQuestions\.length\} \/ \{unit\.questions\.length\}/g, '{unitProgress.attemptedQuestions.length} of {unit.questions.length}');
dbData = dbData.replace(/\{record\.score\} \/ \{record\.totalQuestions\}/g, '{record.score} of {record.totalQuestions}');
fs.writeFileSync(dbPath, dbData);

const vwPath = 'src/components/views/VocabView.tsx';
let vwData = fs.readFileSync(vwPath, 'utf8');
vwData = vwData.replace(/\{masteredIndices\.length\} \/ \{selectedUnit\?\.vocab\.length\}/g, '{masteredIndices.length} of {selectedUnit?.vocab.length}');
fs.writeFileSync(vwPath, vwData);

const mwPath = 'src/components/views/TaskWorksheetView.tsx';
let mwData = fs.readFileSync(mwPath, 'utf8');
mwData = mwData.replace(/\{Object\.keys\(responses\)\.length\} \/ \{task\.worksheetQuestions\?\.length \|\| 0\}/g, '{Object.keys(responses).length} of {task.worksheetQuestions?.length || 0}');
fs.writeFileSync(mwPath, mwData);

const uwPath = 'src/components/views/UserStatsView.tsx';
let uwData = fs.readFileSync(uwPath, 'utf8');
uwData = uwData.replace(/\{masteredCount\} \/ \{totalVocab\}/g, '{masteredCount} of {totalVocab}');
fs.writeFileSync(uwPath, uwData);

console.log('done rewriting strings');
